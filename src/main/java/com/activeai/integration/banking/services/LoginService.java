...
public class LoginService {
  ...
  public ResponseEntity<LoginResponse> getLoginResponseEntity(UserLoginRequest userLoginRequest) {
    ...
    try {
      ...
      ApplicationLogger.logInfo("access token received is ---> " + userLoginRequest.getAccessToken());
      try {
        HttpResponse<String> response = Unirest.get("https://sandbox.apihub.citi.com/gcb/api/v1/accounts")
            .header("Authorization", "Bearer " + userLoginRequest.getAccessToken()).header("Content-Type", "application/json")
            .header("uuid", "12345").header("client_id", "8929a349-d8b3-4584-8cea-95f56499adef").header("Accept", "application/json")
            .asString();
        ApplicationLogger
        ...
        if (Objects.nonNull(response) && StringUtils.isNotEmpty(response.getBody())) {
          ...
          loginResponse = loginResponseMapper.getManipulatedLoginResponse(citiAccountResponse);
          //Here you can call customer profile API and merge the response to login
          ApplicationLogger.logInfo("Login Response Body After Transformation :" + loginResponse);
        }
